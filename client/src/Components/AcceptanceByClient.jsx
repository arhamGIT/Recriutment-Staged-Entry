...
const AcceptanceByClient = () => {
  ...
  const fetchtdata = () => {
    axios.post('/getacceptancesbyclient',{user:user})
      .then((response) => {
  ...
  const submitform = (e) => {
    ...
    if(data.request.length == 0){
      ...
      axios.post('/saveacceptancesbyclient', { data:data, user:user })
      .then((reponse) => {
  ...
  const fetchspecificrequests = (job) => {
    setsl(true)
    axios.post('/getjobspecificsubmitcv', { job: job, user:user })
      .then((response) => {
        if (response.data.status == 200) {
  ...
  const requestselect = (item) => {
    setdata(ps => { return { ...ps, request: item[0] } })
  }
  ...
  const rowclick = (item) => {
    ...
    document.getElementById('validationDefault03').value = item.InterviewDate
  ...
  const updateclick = () => {
    setvalues(true)
    axios.post('/updateacceptancebyclient', { formdata: data })
      .then((response) => {
        if (response.data.status == 200) {
    ...
    <div className="mx-auto my-5 container-fluid" >
      <form className="row g-3" onSubmit={submitform} id="form">
        ...
        <div className="col-md-4">
          <label htmlFor="validationDefault01" className="form-label">Schedule InterView With Client</label>
          <input type="datetime-local" className="form-control" id="validationDefault03" required onChange={e => { setdata(ps => { return { ...ps, InterviewDate: e.target.value } }) }} />
        </div>
        <div className="col-12">
          <button className="btn btn-success" id="savebtn" type="submit">Accepted</button>
      ...
      <div className="mt-5">
        <p className="h3 w-100 text-center">Acceptances By Client</p>
        <div className="cor-3">
...
export default AcceptanceByClient