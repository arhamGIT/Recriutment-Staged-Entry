...
const JobOffered = () => {
  ...
  const fetchtdata = () => {
    axios.post('/getjoboffered',{user:user})
      .then((response) => {
  ...
  const submitform = (e) => {
    ...
    if(data.request.length == 0){
      ...
      axios.post('/savesavejoboffered', { data:data, user:user })
      .then((reponse) => {
  ...
  const fetchspecificrequests = (job) => {
    setsl(true)
    axios.post('/getjobspecificacceptancesbyclient', { job: job, user:user })
      .then((response) => {
        if (response.data.status == 200) {
    ...
    <div className="mx-auto my-5 container-fluid" >
      <form className="row g-3" onSubmit={submitform} id="form">
        ...
        <div className="col-12">
          <button className="btn btn-success" id="savebtn" type="submit">Save JobOffer</button>
      ...
      <div className="mt-5">
        <p className="h3 w-100 text-center">Jobs Offered</p>
        <div className="cor-3">
