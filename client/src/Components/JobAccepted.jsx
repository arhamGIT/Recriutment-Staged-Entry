...
const JobAccepted = () => {
  ...
  const fetchtdata = () => {
    axios.post('/getjobaccepted', { user: user })
      .then((response) => {
  ...
  const submitform = (e) => {
    ...
    if (data.request.length == 0) {
      ...
      axios.post('/savejobaccepted', { data: data, user: user })
        .then((reponse) => {
  ...
  const fetchspecificrequests = (job) => {
    setsl(true)
    axios.post('/getjobspecificjoboffered', { job: job, user: user })
      .then((response) => {
        if (response.data.status == 200) {
    ...
    <div className="mx-auto my-5 container-fluid" >
      <form className="row g-3" onSubmit={submitform} id="form">
        ...
        <div className="col-12">
          <button className="btn btn-success" id="savebtn" type="submit">Save JobAccepted</button>
      ...
      <div className="mt-5">
        <p className="h3 w-100 text-center">Jobs Accepted By Candidate</p>
        <div className="cor-3">
