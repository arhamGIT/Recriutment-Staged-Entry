...
const RecieveAcceptance = () => {
  ...
  const fetchtdata = () => {
    axios.post('/getrecieveacceptance', { user: user })
      .then((response) => {
  ...
  const submitform = (e) => {
    ...
    if (data.request.length == 0) {
      ...
      axios.post('/saverecieveacceptance', { data: data, user: user })
        .then((reponse) => {
  ...
  const fetchspecificrequests = (job) => {
    setsl(true)
    axios.post('/getjobspecificrequests', { job: job, user: user })
      .then((response) => {
        if (response.data.status == 200) {
  ...
  return (
    <div className="mx-auto my-5 container-fluid" >
      ...
      <div className="mt-5">
        <p className="h3 w-100 text-center">Acceptance Recieved</p>
        <div className="cor-3">
...
export default RecieveAcceptance
